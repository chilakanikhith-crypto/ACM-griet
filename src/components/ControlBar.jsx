...
export default function ControlBar({
  ...
  const isGraphAlgo = activeAlgo === 'bfs' || activeAlgo === 'dfs';
  ...
  const handleCustomSubmit = (e) => {
    ...
    if (parsed.length >= 3) {
      ...
      alert("Please enter at least 3 valid positive numbers between 1 and 100 (comma or space separated).");
    }
  };
  ...
  return (
    <div className="bg-slate-900/90 border-y border-slate-800 p-4 backdrop-blur-md shadow-xl transition-all">
      <div className="max-w-7xl mx-auto flex flex-col gap-4">
        ...
        {/* Top Row: Playback Controls & Timeline Scrubber */}
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          
          {/* Main Primary Controls: Play, Pause, Reset, Step */}
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {isPlaying ? (
              ...
              <button
                ...
                title="Play algorithm step-by-step (▶️)"
              >
                <Play className="w-4 h-4 fill-white" />
                <span>{currentStep > 0 && !isComplete ? 'Resume' : 'Play'}</span>
              </button>
            )}
            ...
            <button
              ...
              disabled={isPlaying || (isComplete && currentStep >= totalSteps - 1)}
              className="p-2 rounded-xl bg-slate-800 hover:bg-slate-700 disabled:opacity-40 disabled:cursor-not-allowed text-slate-300 font-semibold text-xs border border-slate-700 active:scale-95 transition-all cursor-pointer"
          ...
          {/* Timeline Scrubber */}
          <div className="flex items-center gap-3 flex-1 lg:max-w-md bg-slate-950/60 px-3.5 py-1.5 rounded-xl border border-slate-800/80">
            <span className="text-xs font-medium text-slate-400 whitespace-nowrap">Timeline:</span>
            <input
              type="range"
              min="0"
            ...
            />
            <div className="text-xs font-mono text-slate-300 whitespace-nowrap bg-slate-850 px-2 py-0.5 rounded">
              <span className="text-cyan-400 font-semibold">{currentStep}</span>
              <span className="text-slate-500"> / </span>
              <span>{Math.max(0, totalSteps - 1)}</span>
            </div>
          </div>

        </div>

        {/* Bottom Row: Context-Specific Algorithm Controls */}
        <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-slate-800/50">
          
          {/* Sorting Specific Controls */}
          {isSorting && (
            <div className="flex items-center gap-2.5 flex-wrap">
              <span className="text-xs font-medium text-slate-400 flex items-center gap-1">
                <Sliders className="w-3.5 h-3.5 text-cyan-400" /> Array:
              </span>
              ...
              {/* Preset types */}
              <button
                onClick={() => onSetPresetArray('reversed')}
                disabled={isPlaying}
                ...
                title="Worst case reversed array"
              >
                Reversed
              </button>
              <button
                onClick={() => onSetPresetArray('nearly_sorted')}
                disabled={isPlaying}
                className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-slate-300 text-xs font-medium border border-slate-700 cursor-pointer"
              ...
              <button
                onClick={() => onSetPresetArray('few_unique')}
                disabled={isPlaying}
                className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-slate-300 text-xs font-medium border border-slate-700 cursor-pointer"
              ...
              <div className="flex items-center gap-2 pl-2 border-l border-slate-800">
                ...
                />
                <span className="text-xs font-mono text-cyan-300">{arraySize}</span>
              </div>

              {/* Custom Input Modal Trigger */}
              <button
          ...
          {/* Binary Search Specific Controls */}
          {isSearch && (
            <div className="flex items-center gap-3 flex-wrap">
              <div className="flex items-center gap-2">
                <span className="text-xs font-medium text-slate-400 flex items-center gap-1">
                  <Search className="w-3.5 h-3.5 text-cyan-400" /> Target Value:
                </span>
                <input
                  type="number"
                  value={targetValue}
                  onChange={(e) => onTargetValueChange(parseInt(e.target.value, 10) || 0)}
                  disabled={isPlaying}
              ...
              {/* Quick Pick from Array */}
              <div className="flex items-center gap-1.5 overflow-x-auto py-1">
                <span className="text-[11px] text-slate-500">Pick:</span>
                {searchArray && searchArray.slice(0, 7).map((val, idx) => (
                  <button
                    key={idx}
                    onClick={() => onTargetValueChange(val)}
                    disabled={isPlaying}
                ...
                {/* Out of bounds target test */}
                <button
                  onClick={() => onTargetValueChange(999)}
                  disabled={isPlaying}
                  ...
                  title="Test element not present in array"
                >
                  999 (Not In Arr)
                </button>
              </div>

              {/* Randomize Search Array */}
              <button
                onClick={onRandomizeArray}
                disabled={isPlaying}
          ...
          {/* Graph / Grid Specific Controls */}
          {isGraphAlgo && (
            <div className="flex items-center gap-3 flex-wrap">
              {/* View Mode Switch: Graph vs Grid */}
              ...
              {viewMode === 'graph' ? (
                <>
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs text-slate-400">Topology:</span>
                    <select
                      value={graphPreset}
                      onChange={(e) => onGraphPresetChange(e.target.value)}
                      disabled={isPlaying}
                    ...
                    >
                      ...
                      <option value="cyclic">Network with Cycles</option>
                      <option value="dag">Complex Web (8 Nodes)</option>
                    </select>
                  </div>
          ...
          <div className="flex items-center gap-2 ml-auto">
            <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold tracking-wide uppercase ${
              ...
              <span className={`w-1.5 h-1.5 rounded-full ${
                isComplete ? 'bg-emerald-400' : isPlaying ? 'bg-cyan-400 animate-ping' : 'bg-slate-500'
              }`} />
              {isComplete ? 'Completed' : isPlaying ? 'Executing' : currentStep > 0 ? 'Paused' : 'Ready'}
            </span>
          </div>
      ...
      {showCustomModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-md w-full shadow-2xl">
            <h3 className="text-lg font-bold text-white mb-2">Input Custom Array</h3>
